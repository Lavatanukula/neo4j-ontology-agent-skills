...
### 3. METRIC (`:Metric`)
Quantifiable business values (e.g., CostPrice, ListPrice, OrderTotal, RefundAmount, StarRating).

**MANDATORY** fields: `id`, `value` (numeric), `type`, `source_quality`.

**METRIC LINKING RULE:** Every Metric node MUST be linked to its owner via `HAS_METRIC`
in the SAME section it is defined. A Metric with no `HAS_METRIC` is an ontology violation.
...
### 4. CONCEPT (`:Concept`)
Abstract taxonomies or categories (e.g., Product Categories, Brand Names).

**MANDATORY types to extract:**
- Product Categories (Electronics, Clothing, Home, …)
- Brand Names — every brand MUST become a Concept node with `type='BrandName'`
- Any named taxonomy the product/entity explicitly belongs to

Every product MUST link to BOTH its category Concept AND its brand Concept via `BELONGS_TO`.
...
Every RelNode MUST connect to a Temporal node.
